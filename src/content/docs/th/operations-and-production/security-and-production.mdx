...
Kafka บน production ซ้อน **TLS** encryption, **SASL/mTLS** authentication และ **ACL** (ผ่าน **`kafka-acls.sh`**) สำหรับ authorization บนแกนที่ durable ทั้ง **replication factor >= 3**, **`min.insync.replicas=2`** และ **`acks=all`** พร้อม **monitoring**, alerting และ **quota** ครบ ยินดีด้วย คุณจบ **Kafka — From Zero to Hero** แล้ว จาก log, broker และ KRaft ผ่าน producer, consumer, delivery semantics, Streams และ Connect ไปจนถึงการรันและทำ Kafka ให้ปลอดภัยบน production ไปสร้างอะไรที่ durable กันเถอะ
</Callout>
