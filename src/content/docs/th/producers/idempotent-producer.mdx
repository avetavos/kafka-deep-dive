...
description: 'idempotence กัน retry ไม่ให้สร้าง duplicate อย่างไรด้วย producer id epoch และ sequence number ทำไมถึงเป็น default ใน Java client แต่ต้อง opt-in เองใน KafkaJS และทำไมถึงไม่เหมือน transactions'
sidebar:
  order: 3
---
...
## idempotence ต้องการอะไร และรับประกันอะไร
...
จุดนี้คนสับสนบ่อย idempotence รับประกันว่า record ถูกเขียน **ครั้งเดียวต่อ partition** แม้มี retry — แต่ทำได้ **แค่นั้น** — **ไม่ได้** ทำให้การ write หลาย record หลาย partition เป็น atomic และ **ไม่ได้** ผูก write ของ producer เข้ากับการ commit offset ของ consumer
...
idempotent producer stamp record ด้วย **producer id**, **epoch** และ **sequence number** เพื่อให้ broker ทิ้ง **duplicate** จาก retry ให้ผล **exactly-once ต่อ partition** วิธีนี้ต้องการ **`acks=all`**, **`retries > 0`** และ **`max.in.flight.requests.per.connection <= 5`** เป็น **default ใน Java client** แต่ใน **KafkaJS คุณต้อง opt in เอง** ด้วย `idempotent: true` — และ **ไม่ใช่** สิ่งเดียวกับ **transactions** เต็มรูปแบบ
</Callout>
...
export const quizIdempotent = [
  ...
  },
  {
    ...
    options: [
      ...
      'ด้วย producer id บวก sequence number ต่อ partition ที่เคยเห็นแล้ว',
