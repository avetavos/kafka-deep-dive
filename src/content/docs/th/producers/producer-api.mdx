...
สร้าง **message** (topic, key, value) โดย `key` และ `value` เป็น string/Buffer ที่คุณ **serialize เอง** (เช่น `JSON.stringify`) แล้วเรียก **`send()`** — เป็น **asynchronous** และคืน **`Promise`** ดังนั้นให้ **`await`** ทุกครั้ง (และใช้ `try`/`catch` จัดการ error) แทนการ block และ connect producer ครั้งเดียวแล้วใช้ซ้ำ
</Callout>
...
export const quizProducerApi = [
  ...
  },
  {
    ...
    options: [
      'block แล้วคืน offset ตรง ๆ เมื่อเขียนเสร็จ',
      'คืน Promise ทันที แล้วทำงานเสร็จแบบ asynchronous เบื้องหลัง',
      'คืน void และไม่เคยรายงานว่าสำเร็จหรือล้มเหลว',
      'คืน byte ดิบที่ส่งไป',
    ],
    answer: 1,
  },
  {
  ...
  },
  {
    ...
    options: [
      'ระบุ broker ทุกตัวที่ต้อง online ไม่งั้นการ send จะล้มเหลว',
      'เป็น list broker เริ่มต้นที่ client ติดต่อเพื่อ discover cluster',
      'ตั้งระยะเวลา retention ของ topic',
      'เลือก partition ที่ record จะลง',
    ],
    answer: 1,
  },
];
