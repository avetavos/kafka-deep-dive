...
tutorial เก่า ๆ จะ start ZooKeeper ก่อนแล้วส่ง `--zookeeper host:2181` เข้า admin command นั่น **ล้าสมัยแล้ว** ใน Kafka 4.x ไม่มี ZooKeeper ให้ start และทุก command ตอนนี้รับ `--bootstrap-server host:9092` ถ้าเห็น `--zookeeper` ในคู่มือ แสดงว่ามาก่อนยุค KRaft
</Callout>
...
## KRaft หมายความว่าอะไรสำหรับคุณในฐานะ developer
...
export const quizKraft = [
  {
    ...
    options: [
      'relational database',
      'KRaft ที่เป็น controller quorum แบบ Raft ที่ built-in อยู่ใน Kafka',
      'Redis',
  ...
  {
    q: 'คุณเห็น tutorial ใช้ `--zookeeper localhost:2181` แปลว่าอะไร',
    options: [
      'เป็น syntax ปัจจุบันที่แนะนำสำหรับ Kafka 4.x',
      'มาก่อนยุค KRaft flag นี้ถูกถอดออกใน Kafka 4.x ให้ใช้ `--bootstrap-server` แทน',
