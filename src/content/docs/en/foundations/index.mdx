...
It targets **Kafka 4.x**, which runs exclusively in **KRaft mode** — ZooKeeper has been fully removed. Every CLI example uses `--bootstrap-server`, and the client examples use the official Java `kafka-clients` API, the canonical reference every other client mirrors.
