...
Production Kafka layers **TLS** encryption, **SASL/mTLS** authentication, and **ACLs** (via **`kafka-acls.sh`**) for authorization on top of a durable core: **replication factor >= 3**, **`min.insync.replicas=2`**, and **`acks=all`**, with **monitoring**, alerting, and **quotas** in place. Congratulations — you have completed the **Kafka Deep Dive**: from logs, brokers, and KRaft, through producers, consumers, delivery semantics, Streams and Connect, to running and securing Kafka in production. Go build something durable.
</Callout>
